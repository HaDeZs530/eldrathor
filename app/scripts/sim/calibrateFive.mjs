...
const WALLS = 10;
const T = { packSec: Number(process.env.PACK_SEC || 18), packLoss: 0.12, levelPerArea: 4.5, levelPower: 0.04, heal: 0.5, wallWin: 0.6 };
...
const tuning = { walls: WALLS, pack: {}, boss: {}, heal: T.heal, xp: {}, gems: false, levelPower: T.levelPower, items: 'five', dropRates: RATES };
...
for (const area of AREAS) {
  ...
  const mid = referenceParty(S, tuning, { tier: area.id, rarity: 'Rare', upg: 5, level: Math.round((lvIn + levelTarget) / 2) });
  const packs = tunePacks(mid, area);
  const arrive = measurePacks(referenceParty(S, tuning, { tier: Math.max(1, area.id - 1), rarity: 'Epic', upg: 10, level: Math.round(lvIn) }), area);
  ...
  const gear = (rarity, upg, level = levelTarget) => referenceParty(S, tuning, { tier: area.id, rarity, upg, level });
  const boss = tuneBoss(gear('Epic', 10), area);
  const wall = { rare10: bossWin(gear('Rare', 10), area), epic0: bossWin(gear('Epic', 0), area), epic10: bossWin(gear('Epic', 10), area), legendary10: bossWin(gear('Legendary', 10), area), mythic10: bossWin(gear('Mythic', 10), area), epic10Under: bossWin(gear('Epic', 10, Math.max(1, levelTarget - 3)), area) };
...
L.push('## 3. The wall — boss win rate by gear, at the area\'s level', '', '| Wall | Rare +10 | Epic +0 | **Epic +10** | Epic +10, 3 levels under | Legendary +10 | Mythic +10 | Boss fight |', '|---|---|---|---|---|---|---|---|');
for (const r of rows) L.push(`| ${r.area} | ${pct(r.wall.rare10)} | ${pct(r.wall.epic0)} | **${pct(r.wall.epic10)}** | ${pct(r.wall.epic10Under)} | ${pct(r.wall.legendary10)} | ${pct(r.wall.mythic10)} | ${r.bossSec} s |`);
