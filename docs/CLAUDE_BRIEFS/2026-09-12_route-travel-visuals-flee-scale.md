...
## Test plan (phone)
Tap a cleared node 8 hops away → marker walks there in ~1 s, clock ticks 8, a respawned node on the path stops it with the ambush card. Flee works and fails visibly (enemies hit first). Party marker is gold, unknown nodes read blue-veiled, rare is the only red. Fight screen text is obviously larger than before; party cards ≥ 120 px; Rally lore is 18 px. Build + tests green.
