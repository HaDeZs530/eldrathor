# Claude briefs

Active coding tasks for **Claude Code**. The **Claude Design Chat** writes these; Claude Code implements.

**Home session:** start at `docs/CLAUDE_HOME_HANDOFF.md`.

- Newest / handoff-listed brief = next build priority unless Anthony says otherwise.
- Filename: `YYYY-MM-DD_short-slug.md`
- When a brief is done, mark **DONE** at the top, move it to `_archive/`, and note it in `docs/SESSION_LOG.md`.

## Queue
- `2026-09-17_ui-brackets-v2.md` — **READY**
- _(empty — every queued brief is done. Waiting on the Design Chat: M2 lock 2 (gems), the 224 weapon
  special names for `app/src/data/weaponNames.js`, and rulings on the DESIGN-OPEN numbers listed in PR #61.)_

## Archive (`_archive/`, newest first — all DONE or superseded)
- `2026-09-17_ui-brackets.md` — DONE (#66)
- `2026-09-17_item-numbers-and-names.md` — DONE (#65)
- `2026-09-16_m2-1-item-model.md` — DONE (#61)
- `2026-09-16_mode-tokens-fix.md` — DONE (#59)
- `2026-09-15_tab-retap-resets-root.md` — DONE (#58)
- `2026-09-14_tuning-pass-2.md` — DONE (#56)
- `2026-09-14_sheets-inherit-mode.md` — DONE (#55)
- `2026-09-14_m1d-engineering-hygiene.md` — DONE (#54)
- `2026-09-14_m1c-afk-true-idle.md` — DONE (#53)
- `2026-09-14_style-bible-chrome.md` — DONE (#52)
- `2026-09-14_m1b-progression-chain.md` — DONE (#51)
- `2026-09-14_m1a-save-and-ids.md` — DONE (#50)
- `2026-09-14_party-ring-marker.md` — DONE (#47)
- `2026-09-14_bugfix-pass-1.md` — DONE (#46)
- `2026-09-13_route-explore-model.md` — DONE (#33)
- `2026-09-12_route-transitions.md` — DONE (#30)
- `2026-09-12_route-polish-2.md` — DONE (#29)
- `2026-09-12_route-travel-visuals-flee-scale.md` — DONE (#23, #26, #27, #28)
- `2026-09-12_route-map-v2.md` — DONE (#21)
- `2026-09-12_ui-shell.md` — DONE (#22)
- `2026-09-11_combat-v2.md` — DONE
- `2026-09-11_loop-cleanup-and-stat-spine.md` — DONE
- `2026-09-11_playtest-ux-polish.md` — DONE
- `2026-09-11_island-dotted-path-10-nodes.md` — DONE
- `2026-09-11_agent-roles-update.md` — DONE (pushed to main by Design Chat)
- `2026-09-11_node-engage-attack-flee.md` — DONE (PR #16) → SUPERSEDED by the loop cleanup
- `2026-09-11_pannable-island-and-node-map.md` — DONE (PR #10)
