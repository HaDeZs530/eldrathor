...
- **Progression baseline needs rulings (Anthony + Design Chat):** `docs/balance/2026-09-19_baseline/FINDINGS.md`. Target is 180–360 h for the mountain (210 h efficient); today the efficient route takes ~70 h, areas 1–6 fall on day one in ~2 h, the only wall is the last boss, late fights last 0.6 s, Worldvein has no sinks (1.4 % of income spent), 526 gems drop per playthrough, the level cap is unreachable. Rule on hours per area and the levers; then Claude Code tunes with the harness (`npm run sim:baseline`) and turns the curve into per-area gates.
