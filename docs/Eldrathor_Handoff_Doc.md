...
## 2. Tech Stack
- **Now:** Vite + React (JavaScript, not TS), single-page app. Plain inline styles/CSS, no UI framework. No backend yet (all client-side state).
- **Dev environment:** Windows, project at `C:\Users\tosie\OneDrive\Worldvein IOS\eldrathor` (Vite scaffold; note OneDrive can cause file-sync friction with node_modules). Node installed. Claude Code (Desktop app, Code tab) is the build/run agent.
